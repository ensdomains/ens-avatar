...
- **Limits** (all configurable on `AvatarResolver`):
  ...
  - `maxSvgLength` — inline / `data:` SVGs: **256 KiB**; SVGs nested deeper than 256 elements or with more than 64 `<style>` elements, and CSS beyond 64 KiB / 2000 nodes per SVG, are dropped.
...
- **Chain adapters throw on outages** (RPC errors, CCIP gateway failures other than 404/410) instead of resolving to `null`. A missing resolver/record still resolves to `null`.
