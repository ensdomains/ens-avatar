...
- **Limits** (all configurable on `AvatarResolver`):
  ...
  - `maxSvgLength` — inline / `data:` SVGs: **256 KiB**; SVGs nested deeper than 256 elements or with more than 64 `<style>` elements, are dropped, as are `<style>` blocks over 64 KiB of raw CSS or 2000 CSS nodes; at most 64 KiB of sanitized CSS is kept per SVG.
